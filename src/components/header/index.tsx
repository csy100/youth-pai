import { Bars3Icon, LanguageIcon, MagnifyingGlassIcon, MoonIcon, SunIcon, UserIcon } from "@heroicons/react/24/outline"
...
function Header() {
  ...
  return (
    <header className="h-14 bg-white dark:bg-zinc-900 flex items-center border-zinc-200 dark:border-zinc-800">
      {/* 左侧 */}
      <div className="w-60 flex items-center ml-5">
        ...
        <span className="text-2xl font-bold text-orange-600 dark:text-orange-400 tracking-tighter">{t('LogoTitle')}</span>
      </div>
      ...
      {/* 右侧 */}
      <div className="flex items-center gap-3 pr-10 justify-end">
        {/* 主题切换按钮加Tooltip */}
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                <MoonIcon
                  className={`w-6 h-6 text-zinc-600 dark:text-yellow-400 absolute transition-opacity transition-transform duration-300
                    ${dark ? 'opacity-100 rotate-0 scale-100' : 'opacity-0 -rotate-90 scale-75'}
                  `}
                />
                <SunIcon
                  className={`w-6 h-6 text-zinc-600 dark:text-yellow-400 transition-opacity transition-transform duration-300
              ...
              <TooltipTrigger asChild>
                <DropdownMenuTrigger asChild>
                  <button className="cursor-pointer p-2 rounded-full hover:bg-zinc-200 dark:hover:bg-zinc-600 transition-colors flex items-center justify-center">
                    <LanguageIcon className="w-6 h-6 text-zinc-600 dark:text-zinc-200" />
                  </button>
                </DropdownMenuTrigger>
              </TooltipTrigger>
              <TooltipContent>
        ...
        <Link to="/login">
          <button className="cursor-pointer p-2 rounded-full hover:bg-zinc-200 dark:hover:bg-zinc-600 transition-colors flex items-center justify-center">
            <UserIcon className="w-6 h-6 text-zinc-600 dark:text-zinc-200" />
          </button>
        </Link>

        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="cursor-pointer ml-3 flex items-center justify-center">
              <span className="inline-flex items-center justify-center w-8 h-8 bg-cyan-200 dark:bg-cyan-700 rounded-full"></span>
            </button>
          </DropdownMenuTrigger>
